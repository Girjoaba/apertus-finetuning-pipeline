#!/usr/bin/env python3
"""
Evaluate Apertus-8B with improved LoRA adapter on MedQA test split.

This script evaluates the fine-tuned Apertus-8B model using the improved
LoRA weights from output/apertus_lora_8B_improved.

Usage:
    python scripts/evaluate_finetuned_8B_improved.py
    python scripts/evaluate_finetuned_8B_improved.py --max_samples 50
"""
...
DEFAULT_ADAPTER = "./output/apertus_lora_8B_improved"
MODEL_NAME = "Apertus-8B + LoRA Improved (local)"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Apertus-8B with improved LoRA adapter on MedQA"
    )
    parser.add_argument(
        "--adapter",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/apertus_8b_lora_improved_test.json",
...
def main():
    ...
    # Setup logging
    log_file = "logs/eval_apertus_8b_lora_improved_detailed.log"
    setup_logging(log_file=log_file)

    logger.info("=" * 70)
    logger.info("MedQA Test Evaluation - Apertus-8B + LoRA Improved (local)")
    logger.info("=" * 70)
